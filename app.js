...
// Cuando entrenes tu modelo en Colab, copia el archivo .task en:
// modelos/modelo_senas.task
const MODELO_PERSONALIZADO = "modelos/modelo_senas.task";
const MODELO_BASICO_MEDIAPIPE =
  "https://storage.googleapis.com/mediapipe-models/gesture_recognizer/gesture_recognizer/float16/1/gesture_recognizer.task";

// Estas claves deben coincidir con los nombres de las carpetas del dataset.
// Ejemplo: dataset_senas/hola/  =>  "hola": "Hola"
const frases = {
  // Señas del modelo personalizado
  "hola": "Hola",
  "gracias": "Gracias",
  "agua": "Quiero agua",
  ...
  "si": "Sí",
  "no": "No",
  "bano": "Necesito ir al baño",
  "comida": "Quiero comer",
  "profesor": "Profesor",
  "estudiante": "Estudiante",

  // Gestos del modelo básico de MediaPipe, usados como respaldo
  "Open_Palm": "Hola",
...
async function cargarModelo() {
  ...
  try {
    recognizer = await GestureRecognizer.createFromOptions(vision, {
      baseOptions: {
        modelAssetPath: MODELO_PERSONALIZADO,
        delegate: "GPU"
      },
      runningMode: "VIDEO",
      numHands: 1
    });
    estado.textContent = "Modelo personalizado cargado correctamente.";
  } catch (error) {
    console.warn("No se encontró el modelo personalizado. Se usará el modelo básico de MediaPipe.", error);

    recognizer = await GestureRecognizer.createFromOptions(vision, {
      baseOptions: {
        modelAssetPath: MODELO_BASICO_MEDIAPIPE,
        delegate: "GPU"
      },
      runningMode: "VIDEO",
      numHands: 1
...
function actualizarContador() {
  if (contadorMuestras) contadorMuestras.textContent = `Muestras capturadas: ${muestras.length}`;
}
...
if (btnCapturar) btnCapturar.addEventListener("click", capturarMuestra);
if (btnDescargarDataset) btnDescargarDataset.addEventListener("click", descargarDataset);
if (btnLimpiarDataset) btnLimpiarDataset.addEventListener("click", limpiarDataset);
...
const btnIrCaptura = document.getElementById("btnIrCaptura");
if (btnIrCaptura) {
  btnIrCaptura.addEventListener("click", () => {
    window.location.href = "./capturar-senas.html";
  });
}
